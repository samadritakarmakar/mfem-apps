...
int main(int argc, char *argv[])
{
   ...
   for (int i=0; i<E.size(); i++)
   {
       ...
       lambda(attrib) = (nu[i]*E[i])/((1+nu[i])*(1-2*nu[i]));
       cout<<"lambda "<<attrib+1<<" = "<<lambda(attrib)<<"\n";
       mu(attrib) = E[i]/(2*(1+nu[i]));
       cout<<"mu "<<attrib+1<<" = "<<mu(attrib)<<"\n";
   }
   //lambda(maxAttr-1) = (nu_1*E_1)/((1+nu_1)*(1-2*nu_1));
   //lambda(maxAttr-2) = (nu_2*E_2)/((1+nu_2)*(1-2*nu_2));
   //cout<<"\nlambda "<<maxAttr<<" = "<<lambda(maxAttr-1)<<"\n";
   //cout<<"lambda "<<maxAttr-1<<" = "<<lambda(maxAttr-2)<<"\n";
   PWConstCoefficient lambda_func(lambda);
   //mu(maxAttr-1) = E_1/(2*(1+nu_1));
   //mu(maxAttr-2) = E_2/(2*(1+nu_2));
   //cout<<"mu "<<maxAttr<<" = "<<mu(maxAttr-1)<<"\n";
   //cout<<"mu "<<maxAttr-1<<" = "<<mu(maxAttr-2)<<"\n";
   ...
   VisItDataCollection visit_dc("Example2", mesh);
   visit_dc.RegisterField("Displacement",&x);
   visit_dc.Save();

   ParaViewDataCollection paraview_dc("PVExample2", mesh);
